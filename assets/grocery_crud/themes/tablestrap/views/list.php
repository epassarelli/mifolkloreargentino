<table cellpadding="0" cellspacing="0" border="0" class="display groceryCrudTable table table-striped" id="<?php echo uniqid(); ?>">
	...
	<tfoot>
		<tr>
			...
			<?php if (!$unset_delete || !$unset_edit || !$unset_read || !empty($actions)) { ?>
				<th>
					<button class="btn btn-default refresh-data btn-block" role="button" data-url="<?php echo $ajax_list_url; ?>">
						<span class="glyphicon glyphicon-refresh"></span> <?php echo $this->l('list_clear_filtering'); ?>
					</button>
				</th>
			<?php } ?>
		</tr>
